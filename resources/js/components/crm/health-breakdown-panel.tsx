...
function formatDetails(details: Factor['details']): string {
    if (!details) return '';
        ...
        <div className="rounded-lg border border-[#1e1e2a] bg-[#0f0f14] p-3">
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <div className="text-xs font-medium text-[#e8e8ed]">
                            Delivery Health
                        </div>
                        <div
                            className="text-[10px]"
            ...
            <div className="space-y-1.5">
                {factors.map((f) => {
                    const pct =
                        f.max_score > 0
                            ? Math.round((f.score / f.max_score) * 100)
                            : 0;
                    return (
                        <Tooltip key={f.key}>
                            ...
                            {f.details && (
                                <TooltipContent
                                    side="bottom"
                                    className="max-w-48 border-[#1e1e2a] bg-[#0f0f14] p-2 text-[10px] text-[#8b8b9e]"
                                >
