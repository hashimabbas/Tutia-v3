...
function groupByDate(events: TimelineEvent[]): Record<string, TimelineEvent[]> {
    ...
    for (const ev of events) {
        ...
        else if (Date.now() - new Date(date).getTime() < 604800000)
            key = 'This Week';
        else
            key = new Date(ev.occurred_at).toLocaleDateString('en-US', {
                month: 'long',
                day: 'numeric',
                year: 'numeric',
            });
...
export default function ActivityTimeline({
    ...
    const handleAddActivity = async (e: React.FormEvent) => {
        ...
        router.post(
            ...
            {
                activitable_type:
                    morphMap[entityType] ?? 'App\\Models\\CrmLead',
        ...
        <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-[#e2e6ef] px-6 py-3">
                <div className="flex items-center gap-2.5">
                    ...
                    {events.length > 0 && (
                        <span className="rounded-md bg-[#e2e6ef] px-1.5 py-0.5 text-[9px] font-medium text-[#6b7280]">
                            {events.length}
                        </span>
                    )}
                </div>
            ...
            {showForm && (
                <form
                    ...
                    className="border-b border-[#e2e6ef] bg-[#f0f2f7] px-6 py-3"
                >
                    <div className="mb-2.5 flex gap-1.5">
                        {['note', 'call', 'email', 'meeting', 'task'].map(
                            (t) => (
                                <button
                                    key={t}
                                    type="button"
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 rounded-lg border border-[#e2e6ef] bg-white px-3 py-1.5 text-xs text-[#1a1a2e] placeholder-[#9ca3af] transition-all outline-none focus:border-[#2B4C8C] focus:ring-[3px] focus:ring-[#2B4C8C]/10"
                        />
                        <button
                            type="submit"
                            disabled={adding}
                ...
                {filterOptions.map((f) => (
                    <button
                        ...
                        className={`rounded-md px-2 py-0.5 text-[10px] capitalize transition-all ${
                            filters.includes(f === 'system' ? 'created' : f) ||
                            (f === 'all' && filters.includes('all'))
                                ? 'bg-[#e2e6ef] font-medium text-[#374151]'
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
                {loading && events.length === 0 ? (
                    ...
                    <>
                        {Object.entries(groupByDate(events)).map(
                            ([dateLabel, dateEvents]) => (
                                <div key={dateLabel} className="mb-5">
                                    <div className="mb-2.5 flex items-center gap-2">
                                        <span className="text-[10px] font-semibold tracking-[0.08em] text-[#6b7280] uppercase">
                                            {dateLabel}
                                        </span>
                                        <span className="h-px flex-1 bg-[#e2e6ef]" />
                                    </div>
                                    <div className="relative space-y-0">
                                        <div className="absolute top-2 left-[15px] h-[calc(100%-16px)] w-px bg-[#e2e6ef]" />
                                        {dateEvents.map((ev) => {
                                            const Icon =
                                                iconMap[ev.icon] ?? FileText;
                                            return (
                                                <div
                                                    key={ev.event_id}
