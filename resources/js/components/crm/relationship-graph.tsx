                        ...
                        <>
                            ...
                            <svg ref={svgRef} viewBox="0 0 280 200" className="mb-3 w-full">
                                {contactNodes.slice(0, 6).map((node, i) => {
                                    ...
                                    const influenceColor = node.influence_type
                                        ? INFLUENCE_COLORS[node.influence_type] ?? '#9ca3af'
                                        : '#9ca3af';
                                    return (
                                        <g key={node.id}>
                                            <line
                                            ...
                                            <circle
                                                ...
                                                stroke={hoveredNode === node.id ? '#1a1a2e' : 'none'}
                                                strokeWidth={1.5}
                                ...
                                <circle cx={CENTER_X} cy={CENTER_Y} r={8} fill="#2B4C8C" fillOpacity={0.85} />
                            ...
                            {Object.keys(influenceSummary).length > 0 && (
                                <div className="mb-3 rounded-xl border border-[#e2e6ef] bg-[#f8f9fc] p-3">
                                    <div className="mb-1.5 text-[9px] font-semibold uppercase tracking-wider text-[#6b7280]">
                                        Influence Map
                                    </div>
                                    <div className="flex flex-wrap gap-1.5">
                                        {Object.entries(influenceSummary).map(([slug, count]) => {
                                            const color = INFLUENCE_COLORS[slug] ?? '#9ca3af';
                                            return (
                                                <span
                                                    key={slug}
                                ...
                                {[...nodes]
                                    ...
                                    .map(node => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#eef1f8] text-[10px] font-medium text-[#6b7280]"
                                                >
                                                    {node.name.charAt(0)}
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-xs font-medium text-[#1a1a2e]">{node.name}</span>
                                                            ...
                                                            <div
                                                                ...
                                                                style={{
                                                                    ...
                                                                    backgroundColor: node.influence_type
                                                                        ? INFLUENCE_COLORS[node.influence_type] ?? '#2B4C8C'
                                                                        : '#2B4C8C',
                                                                }}
                                                            />
                                                        </div>
                                                ...
                                                {weight > 0 && (
                                                    <span className="text-[10px] font-medium text-[#6b7280]">{weight}</span>
                                                )}
                                            </div>
                                        );
                                    })}
                ...
                {orgRelationships.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                        {orgRelationships.map((rel, i) => {
                            const strengthColor = rel.strength >= 80 ? '#10b981' : rel.strength >= 50 ? '#f59e0b' : '#ef4444';
                            return (
                                <div
                                    key={i}
                                    className="flex items-center gap-2.5 rounded-xl border border-[#e2e6ef] bg-white px-3 py-2.5 transition-all hover:border-[#c8cce0] hover:shadow-sm"
                                >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="text-xs font-medium text-[#1a1a2e]">{rel.source_name ?? rel.target_name}</div>
                                        <div className="text-[10px] text-[#6b7280] capitalize">{rel.type_name}</div>
                                    </div>
                                    {rel.strength > 0 && (
                                        <div className="flex flex-col items-end gap-0.5">
                                            <span className="text-[10px] font-medium text-[#6b7280]">{rel.strength}%</span>
