...
function parseInitialRows(
    ...
    const tokens = expression.match(
        /(?:NOT\s*\([^)]+\)|[A-Za-z_.]+(?:\s*[!=<>]+\s*"[^"]*"|[A-Za-z_.]+\s*[!=<>]+\s*[^\s"]+)|AND|OR)/g,
    );
    if (!tokens) return [];
...
function parseSingleRule(
    ...
    const match = inner.match(
        /^([A-Za-z_.]+)\s+(=|!=|>=|<=|>|<|in|not_in|contains|starts_with|ends_with)\s+(.+)$/,
    );
...
export default function ExpressionBuilder({
    ...
    const [rows, setRows] = useState<ExpressionRule[]>(() =>
        parseInitialRows(expression, fields),
    );
    const [validation, setValidation] = useState<ValidationResult | null>(null);
    const debounceRef = useRef<ReturnType<typeof setTimeout>>();

    const convertToExpression = useCallback(
        (currentRows: ExpressionRule[]): string | null => {
    ...
    const convertRowsToExpression = useCallback(
        async (currentRows: ExpressionRule[]) => {
            ...
            if (expr) {
                try {
                    const res = await fetch(
                        '/crm/workflows/expression/validate',
                        {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
    ...
    return (
        <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                            ...
                            {getFieldType(row.field) === 'boolean' ? (
                                ...
                                <input
                                    type={
                                        getFieldType(row.field) === 'numeric'
                                            ? 'number'
                                            : 'text'
                                    }
                                    value={String(row.value)}
            ...
            <button
                type="button"
                onClick={addRow}
                className="text-sm font-medium text-[#2B4C8C] hover:text-[#3b5d9c]"
            >
                + Add Condition
            </button>
