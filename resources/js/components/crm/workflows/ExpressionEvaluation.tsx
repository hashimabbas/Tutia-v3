...
function isNewFormat(rules: MatchedRule[] | string[]): rules is MatchedRule[] {
    return (
        rules.length > 0 && typeof rules[0] === 'object' && 'rule' in rules[0]
    );
}
...
export default function ExpressionEvaluation({
        ...
        return (
            <div className="rounded-xl border border-gray-200 bg-white p-6 text-center shadow-sm">
                <p className="text-xs text-gray-500">
                    No expression evaluation data available for this run.
                </p>
                <p className="mt-1 text-[10px] text-gray-400">
                    Expression evaluation results are only captured for v2
    ...
    const rules =
        matchedRules && matchedRules.length > 0
            ? isNewFormat(matchedRules)
                ? matchedRules
                : matchedRules.map((r) => ({ rule: r, passed }))
            : [];

    const rulesCount = rules.length;
    const rulesPassed = rules.filter((r) => r.passed).length;
    const totalRules = evaluatedRules || rulesCount;
    const totalPassed = hasTrace
        ? trace.filter((t) => t.passed).length
        : rulesPassed;
