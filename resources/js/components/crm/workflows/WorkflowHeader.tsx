        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            className="flex-1 rounded-md border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-900 outline-none focus:border-[#3b6cdb] focus:ring-1 focus:ring-[#3b6cdb]/20"
                            placeholder="Workflow name"
                        />
                        <span className={cn(
                            'inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-medium',
                            initial.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500',
                        )}>
                        ...
                        <span className="rounded bg-gray-100 px-2 py-0.5 text-[10px] text-gray-500">v{initial.version}</span>
                    </div>

                    <textarea
                        value={description}
                        onChange={e => setDescription(e.target.value)}
                        className="w-full rounded-md border border-gray-200 bg-white px-3 py-1.5 text-xs text-gray-600 outline-none focus:border-[#3b6cdb] focus:ring-1 focus:ring-[#3b6cdb]/20"
                    ...
                    <div className="flex items-center gap-3">
                        <select
                            ...
                            className="rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-xs text-gray-600 outline-none focus:border-[#3b6cdb] focus:ring-1 focus:ring-[#3b6cdb]/20"
                        >
                ...
                <div className="flex items-center gap-1">
                    <button onClick={handleToggle} className="rounded p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700" title={initial.is_active ? 'Deactivate' : 'Activate'}>
                        {initial.is_active ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}
                    </button>
                    <button onClick={handleDuplicate} className="rounded p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700" title="Duplicate">
                        <Copy className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={handleDelete} className="rounded p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-red-500" title="Delete">
