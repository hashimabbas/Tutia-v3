...
const priorityConfig = {
    ...
    medium: { icon: AlertTriangle, color: 'text-[#d97706]', bg: 'bg-amber-100' },
        ...
        <div className="group relative rounded-lg border border-border/60 bg-white p-3 shadow-xs transition-all duration-200 hover:border-border hover:shadow-sm">
            <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                    <div className="pr-5 text-xs font-semibold text-foreground">{title}</div>
            ...
            {onDismiss && (
                <button
                    onClick={onDismiss}
                    className="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded text-muted-foreground/40 opacity-0 transition-all duration-200 hover:bg-muted hover:text-muted-foreground group-hover:opacity-100"
                >
                    <X className="h-3 w-3" />
                </button>
            )}
