...
function ApproveButton({
    ...
    return (
        <button
            ...
            className="rounded bg-[#22c55e] px-2.5 py-1 text-[11px] font-medium text-white transition-colors hover:bg-[#16a34a]"
        >
            Approve
        </button>
    );
...
function RejectButton({
    ...
    return (
        <button
            ...
            className="rounded border border-[#f87171]/30 px-2.5 py-1 text-[11px] font-medium text-[#f87171] transition-colors hover:bg-[#f87171]/10"
        >
            Reject
        </button>
    );
...
export default function PortalChangeOrders({
    ...
    const totalImpact = change_orders
            ...
            <div className="space-y-4 p-4 pb-20 md:p-6">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-lg border border-[#1e1e2a] bg-[#0f0f14] p-4">
                        ...
                        <div className="mt-1 text-lg font-bold text-[#e8e8ed]">
                            {change_orders.length}
                        </div>
                    </div>
                    <div className="rounded-lg border border-[#eab308]/20 bg-[#0f0f14] p-4">
                        <div className="text-[11px] text-[#eab308]">
                            Pending
                        </div>
                        <div className="mt-1 text-lg font-bold text-[#e8e8ed]">
                            {pendingCOs.length}
                    ...
                    <div className="rounded-lg border border-[#1e1e2a] bg-[#0f0f14] p-4">
                        ...
                        <div className="mt-1 text-lg font-bold text-[#22c55e]">
                ...
                {change_orders.length === 0 ? (
                    <div className="rounded-lg border border-[#1e1e2a] bg-[#0f0f14] p-8">
                        <PortalEmptyState
                            ...
                            description="Approved or pending changes will appear here."
                        />
                    </div>
                ) : (
                    <div className="space-y-2">
                        {change_orders.map((co) => {
                            ...
                            return (
                                <div
                                    key={co.id}
                                    className="flex items-center gap-4 rounded-lg border border-[#1e1e2a] bg-[#0f0f14] p-4"
                                >
                                    <div
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-[13px] font-medium text-[#e8e8ed]">
                                            {co.title}
                                        </p>
                                    ...
                                    <span className="rounded-full bg-[#1a1a24] px-2 py-0.5 text-[10px] text-[#555570] capitalize">
                                        {co.status}
                                    </span>
                                    {co.status === 'pending' &&
                                        co.project_id && (
