...
const benefits = [
    { icon: Clock, en: 'Fast Response', ar: 'رد سريع', descEn: 'We respond within 24 hours', descAr: 'نرد عليك خلال 24 ساعة' },
    { icon: Award, en: 'Expert Team', ar: 'فريق خبير', descEn: 'Certified professionals with years of experience', descAr: 'محترفون معتمدون بخبرة سنوات' },
    { icon: Shield, en: 'No Obligation', ar: 'بدون التزام', descEn: 'Free consultation with no commitment required', descAr: 'استشارة مجانية بدون أي التزام' },
];
...
export default function Consultation() {
    ...
    return (
        <>
            <Head title={locale === 'ar' ? 'احجز استشارة مجانية' : 'Book a Free Consultation'}>
                <meta name="description" content={locale === 'ar' ? 'احجز استشارة مجانية مع فريق توتيا التقني' : 'Book a free consultation with the TUTIA technical team'} />
            </Head>
                            ...
                            <form onSubmit={handleSubmit} className="rounded-xl border border-neutral-200 bg-white p-6 md:p-8">
                                <div className="grid gap-6 md:grid-cols-2">
                                    ...
                                    <div className="space-y-2 md:col-span-2">
                                        <Label htmlFor="time_slot">{locale === 'ar' ? 'الوقت المفضل' : 'Preferred Time'} *</Label>
                                        <Select
                                            value={data.time_slot}
                                            onValueChange={(v) => setData('time_slot', v)}
                                        >
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder={locale === 'ar' ? 'اختر الوقت المناسب' : 'Select a time slot'} />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {timeSlots.map((slot) => (
                                                    <SelectItem key={slot.value} value={slot.value}>
                                                        {locale === 'ar' ? slot.ar : slot.en}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    ...
                                    <div className="space-y-2 md:col-span-2">
                                        <Label htmlFor="description">{locale === 'ar' ? 'وصف المشروع' : 'Project Description'} *</Label>
                                        <textarea
                                            id="description"
                                            rows={4}
                                            value={data.description}
                                            onChange={(e) => setData('description', e.target.value)}
                                            placeholder={locale === 'ar' ? 'صف مشروعك بإيجاز' : 'Briefly describe your project'}
                                ...
                                >
                                    {locale === 'ar' ? 'احجز الاستشارة' : 'Book Consultation'}
                        ...
                        <div className="md:col-span-2">
                            <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-6 md:p-8">
                                <Heading level="h3" className="text-brand-navy-900">
                                    {locale === 'ar' ? 'لماذا تحجز مع توتيا؟' : 'Why book with TUTIA?'}
                                </Heading>
                                            ...
                                            <div key={i} className="flex gap-4">
                                                ...
                                                <div>
                                                    <Text variant="body" className="font-semibold text-neutral-900">
                                                        {locale === 'ar' ? benefit.ar : benefit.en}
                                                    </Text>
                                ...
                                <div className="mt-8 border-t border-neutral-200 pt-6">
                                    ...
                                    <div className="mt-4 space-y-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Text variant="body-sm" className="text-neutral-600">+249 123 456 789</Text>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <Mail className="size-4 text-brand-navy-500" />
                                            <Text variant="body-sm" className="text-neutral-600">info@tutia.sd</Text>
                                        </div>
                                    </div>
                                </div>
                            </div>
