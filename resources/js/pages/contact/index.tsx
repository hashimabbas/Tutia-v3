...
import { Calendar, FileText, Headset, DollarSign, Phone, Mail, MapPin, ArrowRight } from 'lucide-react';
...
const cards = [
    {
        ...
        en: { title: 'Book a Free Consultation', desc: 'Schedule a no-obligation consultation with our experts' },
        ar: { title: 'احجز استشارة مجانية', desc: 'احجز استشارة بدون التزام مع خبرائنا' },
    },
    {
        icon: FileText,
        href: '/contact/proposal',
        en: { title: 'Request a Proposal', desc: 'Get a tailored proposal for your project' },
        ar: { title: 'طلب عرض سعر', desc: 'احصل على عرض سعر مخصص لمشروعك' },
    },
    {
        icon: Headset,
        href: '/contact/sales',
        en: { title: 'Contact Sales', desc: 'Speak with our sales team directly' },
        ar: { title: 'تحدث إلى المبيعات', desc: 'تحدث مع فريق المبيعات مباشرة' },
    },
    {
        icon: DollarSign,
        href: '/contact/quote',
        en: { title: 'Get a Quick Quote', desc: 'Receive a quick estimate for your needs' },
        ar: { title: 'احصل على عرض سعر سريع', desc: 'احصل على تقدير سريع لاحتياجاتك' },
    },
];
...
export default function ContactIndex() {
    ...
    const contactItems = [
        { icon: Phone, labelEn: 'Phone', labelAr: 'الهاتف', value: COMPANY_INFO.phone[0] },
        { icon: Mail, labelEn: 'Email', labelAr: 'البريد الإلكتروني', value: COMPANY_INFO.email },
        { icon: MapPin, labelEn: 'Address', labelAr: 'العنوان', value: locale === 'ar' ? COMPANY_INFO.addressAr : COMPANY_INFO.address },
    ];

    return (
        <>
            <Head title={locale === 'ar' ? 'اتصل بنا' : 'Contact Us'}>
                <meta name="description" content={locale === 'ar' ? 'تواصل مع فريق توتيا' : 'Get in touch with the TUTIA team'} />
            </Head>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <h3 className="mt-4 text-lg font-semibold text-neutral-900 group-hover:text-brand-navy-500">
                                        {locale === 'ar' ? card.ar.title : card.en.title}
                                    </h3>
                                    <Text variant="body-sm" className="mt-2 text-neutral-600">
                                        {locale === 'ar' ? card.ar.desc : card.en.desc}
                                    </Text>
                ...
                <Container>
                    ...
                    <div className="mt-10 grid gap-6 md:grid-cols-3">
                        {contactItems.map((item) => {
                            ...
                            return (
                                <div key={item.labelEn} className="flex flex-col items-center rounded-xl border border-neutral-200 bg-white p-6 text-center">
                                    ...
                                    <Text variant="body" className="mt-4 font-semibold text-neutral-900">
            ...
            <Section background="navy">
                <Container className="mx-auto max-w-3xl text-center">
                    <Heading level="h2" className="text-white">
                        {locale === 'ar' ? 'لست متأكداً من أين تبدأ؟' : 'Not sure where to start?'}
                    </Heading>
                    <Text variant="body-lg" className="mt-4 text-neutral-300">
                        {locale === 'ar'
                    ...
                    <div className="mt-8">
                        <Button asChild size="lg" className="bg-brand-gold-500 text-white hover:bg-brand-gold-600">
                            <Link href="/contact/sales">
                                {locale === 'ar' ? 'تحدث إلى فريقنا' : 'Talk to our team'}
