...
import { ArrowLeft, Building2, Globe, Phone, Mail, Users, DollarSign, Activity, Tag, Plus, UserPlus, Briefcase, ChevronDown, MoreHorizontal, Edit, Trash2, Send, Target, User, StickyNote, PhoneCall, MapPin } from 'lucide-react';
...
export default function OrganizationShow({ organization: org, health_score, recommendations, relationship_graph }: Props) {
    ...
    const handleLogQuickNote = (e: React.FormEvent) => {
        ...
        router.post('/crm/activities', {
            ...
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => { setNoteText(''); setQuickAction(null); setSavingNote(false); },
            ...
            <div className="flex h-full flex-col bg-[#f8f9fc]">
                ...
                <div className="sticky top-0 z-20 border-b border-[#e2e6ef] bg-white/90 backdrop-blur-xl">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <h1 className="text-lg font-semibold text-[#1a1a2e] tracking-tight">{org.name}</h1>
                                        {health_score && (
                                            <HealthScoreBadge score={health_score.score} tier={health_score.tier} size="sm" trend={health_score.trend} factors={health_score.factors} />
                                        )}
                                    </div>
                                    <p className="flex items-center gap-1.5 text-xs text-[#6b7280]">
                                        ...
                                        {org.industry && <><span className="text-[#d1d5db]">·</span><span>{org.industry}</span></>}
                                        {org.size && <><span className="text-[#d1d5db]">·</span><span>{org.size}</span></>}
                                        {org.owner && <><span className="text-[#d1d5db]">·</span><span className="inline-flex items-center gap-1"><User className="h-3 w-3" />{org.owner.name}</span></>}
                                    </p>
                                    {org.classifications.length > 0 && (
                                        <div className="mt-1 flex gap-1">
                                            {org.classifications.map(c => (
                                                <span key={c.id} className="rounded-md bg-[#eef1f8] px-1.5 py-0.5 text-[9px] font-medium text-[#6b7280]">{c.name}</span>
                                            ))}
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="flex items-center gap-2">
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-40 border-[#e2e6ef] bg-white text-xs text-[#1a1a2e] shadow-lg">
                                    <DropdownMenuLabel className="text-[10px] font-medium uppercase tracking-wider text-[#6b7280]">Manage</DropdownMenuLabel>
                                    <DropdownMenuSeparator className="bg-[#e2e6ef]" />
                                    <DropdownMenuItem onClick={() => router.visit(`/crm/organizations/${org.id}/edit`)} className="cursor-pointer focus:bg-[#eef1f8]">
                    ...
                    <div className="grid grid-cols-4 gap-px border-t border-[#e2e6ef] bg-[#e2e6ef]">
                        {[
                            { label: 'Active Deals', value: activeDeals.length, icon: Briefcase, gradient: 'from-amber-500 to-amber-600' },
                            { label: 'Pipeline Value', value: formatCurrency(pipelineValue), icon: DollarSign, gradient: 'from-emerald-500 to-emerald-600' },
                            { label: 'Stakeholder Coverage', value: `${stakeholderCoverage}%`, icon: Users, gradient: 'from-violet-500 to-violet-600' },
                            { label: 'Relationship Strength', value: relationshipStrength, icon: Target, gradient: 'from-blue-500 to-blue-600' },
                        ].map((stat) => (
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    >
                        <div className={cn('h-8 w-0.5 rounded-full bg-[#e2e6ef] transition-all', leftPanelOpen && 'rotate-0')} />
                    </button>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            {recommendations.length > 0 && (
                                <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                    <div className="mb-3 flex items-center justify-between">
                                        <h2 className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6b7280]">Recommendations</h2>
                                    </div>
                            ...
                            <div className="border-b border-[#e2e6ef] bg-white px-6 py-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    {quickAction && (
                                        <button onClick={() => setQuickAction(null)} className="text-[10px] text-[#9ca3af] hover:text-[#6b7280]">Cancel</button>
                                    )}
                                </div>
                                {!quickAction ? (
                                    <div className="mt-2 flex flex-wrap gap-1.5">
                                        <button onClick={() => setQuickAction('note')} className="inline-flex items-center gap-1.5 rounded-lg border border-[#e2e6ef] bg-white px-3 py-1.5 text-[11px] text-[#6b7280] shadow-sm transition-all hover:border-[#c8cce0] hover:text-[#374151] hover:shadow-md">
                                            <StickyNote className="h-3 w-3" />
                                            Quick Note
                                        </button>
                                    </div>
                                ) : (
                                    <form onSubmit={handleLogQuickNote} className="mt-2">
                                        <input
                                            type="text"
                                            value={noteText}
                                        ...
                                        <div className="flex justify-end gap-1.5">
                                            <button type="button" onClick={() => setQuickAction(null)} className="rounded-lg border border-[#e2e6ef] bg-white px-3 py-1.5 text-[11px] text-[#6b7280] hover:bg-[#f8f9fc]">Cancel</button>
                                            <button type="submit" disabled={savingNote || !noteText.trim()} className="rounded-lg bg-[#2B4C8C] px-3 py-1.5 text-[11px] font-medium text-white shadow-sm transition-all hover:bg-[#2B4C8C]/90 hover:shadow-md disabled:opacity-50">
                            ...
                            <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                ...
                                {org.contacts.length === 0 ? (
                                    <div className="rounded-xl border border-dashed border-[#e2e6ef] p-6 text-center">
                                        ...
                                        <p className="text-xs text-[#6b7280]">No contacts linked yet</p>
                                        <p className="mt-1 text-[10px] text-[#9ca3af]">Add contacts to this organization to track relationships</p>
                                    </div>
                                ) : (
                                        ...
                                        {org.contacts.map(contact => (
                                            ...
                                            >
                                                <Avatar className="h-8 w-8 rounded-lg">
                                                    <AvatarFallback className="bg-gradient-to-br from-gray-100 to-gray-50 text-xs font-medium text-[#1a1a2e] rounded-lg">
                                                        {contact.first_name.charAt(0)}{contact.last_name.charAt(0)}
                                                    </AvatarFallback>
                                                </Avatar>
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-xs font-medium text-[#1a1a2e]">{contact.name}</span>
                                                        {contact.pivot.is_primary && (
                            ...
                            <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                ...
                                {org.deals.length === 0 ? (
                                    <div className="rounded-xl border border-dashed border-[#e2e6ef] p-6 text-center">
                                        ...
                                        <p className="text-xs text-[#6b7280]">No deals yet</p>
                                    </div>
                                ) : (
                                    <div className="space-y-1.5">
                                        {org.deals.map(deal => (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="mt-0.5 flex items-center gap-1.5 text-[10px] text-[#6b7280] capitalize">
                                                            ...
                                                            {deal.owner && <><span>·</span>{deal.owner.name}</>}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="text-sm font-semibold text-[#1a1a2e]">{formatCurrency(deal.value)}</div>
                                                </div>
                                            </Link>
                                        ))}
                                    </div>
                            ...
                            {org.notes && (
                                <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                    ...
                                    <div className="rounded-xl border border-[#e2e6ef] bg-[#f8f9fc] p-4 shadow-sm">
                                        <p className="text-xs leading-relaxed text-[#6b7280]">{org.notes}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {org.tags.length > 0 && (
                                <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                    <h2 className="mb-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6b7280]">Tags</h2>
                            ...
                            {org.addresses.length > 0 && (
                                <div className="border-b border-[#e2e6ef] bg-white px-6 py-4">
                                    <h2 className="mb-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#6b7280]">Addresses</h2>
                                    <div className="space-y-1.5">
                                        {org.addresses.map(addr => (
                                            <div key={addr.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs text-[#6b7280] hover:bg-[#f8f9fc]">
