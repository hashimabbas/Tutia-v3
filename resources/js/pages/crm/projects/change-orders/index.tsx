...
export default function ChangeOrderIndex({
    project,
    changeOrders,
    filters,
}: Props) {
    ...
    const applyFilter = (key: string, val: string) => {
        router.get(
            `/crm/projects/${project.id}/change-orders`,
    ...
    const handleApprove = (co: ChangeOrder) => {
        router.post(
            `/crm/projects/${project.id}/change-orders/${co.id}/approve`,
            {},
    ...
    const handleReject = (co: ChangeOrder) => {
        ...
        if (reason !== null) {
            router.post(
                `/crm/projects/${project.id}/change-orders/${co.id}/reject`,
    ...
    const pendingCOs = changeOrders.data.filter(
        (co) => co.status === 'identified',
    );
    const approvedCOs = changeOrders.data.filter(
