...
export default function WorkflowRunShow({ run }: Props) {
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                    <div>
                        ...
                        <span
                            className={cn(
                                'mt-1 inline-block rounded-full px-2.5 py-0.5 text-xs capitalize',
            ...
            {run.status === 'failed' && run.rootCause && (
                <div className="rounded-lg border border-red-200 bg-red-50/50 p-4">
                    <p className="text-[10px] tracking-wider text-red-600 uppercase">
                        Root Cause
                    </p>
                    <p className="mt-1 text-sm text-gray-900">
                        {run.rootCause}
                    ...
                    >
                        <div className="mb-3 flex items-center justify-between">
                            <div>
                                <p className="text-xs font-medium text-gray-900">
                                    {req.flow?.name ??
                                        `Approval Flow #${req.approvalFlowId}`}
                                </p>
                                <p className="text-[10px] text-gray-500">
                                    Strategy: {req.flow?.strategy ?? '\u2014'}
                                </p>
                            </div>
                            <span
                                className={cn(
                        ...
                        {req.resolutionTimeMinutes !== null && (
                            <div className="mb-3">
                                ...
                                <p className="text-xs text-gray-900">
                                    {req.resolutionTimeMinutes} minutes
                                </p>
                            </div>
                        )}
                        ...
                        {req.decisions.length > 0 && (
                            <div>
                                <p className="mb-2 text-[10px] tracking-wider text-gray-500 uppercase">
                                    Decisions
                                </p>
                                <div className="space-y-1">
                                    {req.decisions.map((d) => (
                                        <div
                                            key={d.id}
                                            className="flex items-center justify-between rounded bg-gray-50/50 px-3 py-1.5"
                                        >
    ...
    const getExpressionDebugProps = () => {
        ...
        return {
            ...
            matchedRules:
                snapshot?.event_payload?._expression_matched_rules ?? null,
            trace: snapshot?.event_payload?._expression_trace ?? null,
            evaluatedRules:
                snapshot?.event_payload?._expression_evaluated_rules ?? 0,
            passedRules: snapshot?.event_payload?._expression_passed_rules ?? 0,
        };
    };

    const renderEvaluation = () => {
        const { expression, matchedRules, trace, evaluatedRules, passedRules } =
            getExpressionDebugProps();
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-col bg-gray-50/30">
                ...
                <div className="sticky top-0 z-10 border-b border-gray-200 bg-white/90 px-6 py-3 backdrop-blur-xl">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                {run.workflow?.name} {'\u00b7'} {run.eventKey}
                            </p>
                        </div>
                        <span
                            className={cn(
                ...
                <div className="sticky top-0 z-10 border-b border-gray-200 bg-white/90 px-6 backdrop-blur-xl">
