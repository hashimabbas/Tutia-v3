...
function ProbabilityBadge({ probability }: { probability: number }) {
    ...
    return <span className={cn('rounded-md px-1.5 py-0.5 text-[10px] font-medium', color)}>{probability}%</span>;
}
...
function StageDot({ stage }: { stage: string }) {
    ...
    return <span className={cn('h-1.5 w-1.5 rounded-full shrink-0', colors[stage] ?? 'bg-slate-400')} />;
}
        ...
        >
            <div className="mb-2.5 flex items-start justify-between">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <h3 className="text-sm font-medium text-[#1a1a2e] truncate">{deal.title}</h3>
                </div>
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button
                            onClick={(e) => e.preventDefault()}
                            className="flex h-6 w-6 items-center justify-center rounded-lg text-[#9ca3af] opacity-0 transition-all hover:bg-[#f0f2f7] hover:text-[#6b7280] group-hover:opacity-100"
                        >
                    ...
                    <DropdownMenuContent align="end" className="w-36 text-xs">
                        <DropdownMenuItem asChild className="cursor-pointer">
                            <Link href={`/crm/deals/${deal.id}/edit`}>Edit</Link>
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={handleDelete} disabled={deleting} className="cursor-pointer text-rose-600 focus:text-rose-700 focus:bg-rose-50">
            ...
            {deal.company && (
                <p className="mb-1 text-[11px] text-[#6b7280]">{deal.company}</p>
            )}

            <div className="flex items-center justify-between">
                {deal.expected_close_date ? (
                    <span className="text-[11px] text-[#9ca3af]">
                        Close: {new Date(deal.expected_close_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                ) : (
                    <span className="text-[11px] text-[#d1d5db]">No close date</span>
                )}
                {deal.owner && (
                    <span className="text-[10px] text-[#9ca3af]">{deal.owner.name}</span>
                )}
            </div>
        </Link>
    );
        ...
        <>
            ...
            <div className="flex h-full flex-col bg-[#f8f9fc]">
                ...
                <div className="flex items-center justify-between border-b border-[#e2e6ef] bg-white/90 backdrop-blur-xl px-6 py-4">
                    <div>
                        <h1 className="text-lg font-semibold text-[#1a1a2e] tracking-tight">Deals</h1>
                ...
                <div className="flex items-center gap-3 border-b border-[#e2e6ef] bg-white px-6 py-3">
                    <form onSubmit={handleSearch} className="relative">
                        ...
                        <input
                            ...
                            className="h-9 w-56 rounded-xl border border-[#e2e6ef] bg-[#f8f9fc] pl-9 pr-3 text-xs text-[#1a1a2e] placeholder-[#9ca3af] outline-none transition-all focus:border-[#2B4C8C] focus:ring-[3px] focus:ring-[#2B4C8C]/10"
                        />
                    </form>
                </div>
                                    ...
                                    <div className="flex flex-col gap-2.5">
                                        ...
                                        {group.deals.length === 0 && (
                                            <div className="rounded-xl border border-dashed border-[#e2e6ef] bg-white/50 px-4 py-10 text-center">
                                                <p className="text-xs text-[#9ca3af]">No deals</p>
                                                <p className="text-[10px] text-[#d1d5db] mt-0.5">Drag or add new</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
