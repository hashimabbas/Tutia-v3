...
export default function Leadership() {
    ...
    return (
        <>
            <Head title={locale === 'ar' ? 'الإدارة - توتيا' : 'Leadership - TUTIA'}>
                <meta
                    name="description"
                    content={locale === 'ar' ? 'تعرف على فريق القيادة في توتيا' : 'Meet the leadership team at TUTIA'}
                />
            </Head>
                        ...
                        {LEADERS.map((leader) => (
                            ...
                            >
                                <div className="flex items-start gap-5">
                                    ...
                                    <div>
                                        <Heading level="h3">
                                            {locale === 'ar' ? leader.nameAr : leader.name}
                                        </Heading>
                                        <Text variant="body-sm" className="mt-1 font-medium text-brand-navy-500">
                                            {locale === 'ar' ? leader.titleAr : leader.title}
                                        </Text>
                                        <Text variant="body" className="mt-3 text-neutral-600">
                                            {locale === 'ar' ? leader.bioAr : leader.bio}
                                        </Text>
                                    </div>
                                </div>
                            </div>
            ...
            <Section background="muted">
                <Container className="mx-auto max-w-3xl text-center">
                    <Heading level="h2">
                        {locale === 'ar' ? 'تواصل مع فريقنا' : 'Talk to Our Team'}
                    </Heading>
