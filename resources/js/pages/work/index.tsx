...
const CASE_STUDY_TITLES: Record<string, { title: string; titleAr: string }> = {
    'matger-tutia': { title: 'Matger-TUTIA Platform', titleAr: 'منصة متجر توتيا' },
    'erp-implementation': { title: 'ERP Implementation', titleAr: 'تطبيق نظام تخطيط الموارد' },
    'connectivity-project': { title: 'Connectivity Project', titleAr: 'مشروع الاتصال' },
};
            ...
            <Section>
                <Container>
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {CASE_STUDIES.map((cs) => {
                            const titles = CASE_STUDY_TITLES[cs.slug] ?? { title: cs.client, titleAr: cs.clientAr };
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <h3 className="mt-4 text-lg font-semibold text-neutral-900 group-hover:text-brand-navy-500">
                                        {locale === 'ar' ? titles.titleAr : titles.title}
                                    </h3>
                                    <Text variant="body-sm" className="mt-2 text-neutral-600">
                                        {locale === 'ar' ? cs.summaryAr : cs.summary}
                                    </Text>
                                    <div className="mt-4">
                                        <span className="block text-2xl font-bold text-brand-navy-700">
                                            {locale === 'ar' ? cs.resultMetricAr : cs.resultMetric}
                                        </span>
                                        <Text variant="caption" className="text-neutral-500">
                                            {locale === 'ar' ? cs.resultLabelAr : cs.resultLabel}
                                        </Text>
                                    </div>
