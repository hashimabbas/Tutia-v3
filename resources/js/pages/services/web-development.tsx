...
import { Globe, ArrowRight, ArrowLeft, Search, Code, Sparkles, RefreshCw } from 'lucide-react';
...
export default function WebDevelopment() {
    ...
    const phases = locale === 'ar'
        ? [
            { icon: Sparkles, title: 'تصميم', desc: 'تصميم إبداعي حائز على جوائز يلفت انتباه جمهورك' },
            { icon: Search, title: 'تحسين محركات البحث', desc: 'تحسينSEO لضمان ظهور موقعك في صدارة نتائج البحث' },
            { icon: Code, title: 'تطوير', desc: 'بناء المواقع بأحدث تقنيات الويب وفق أعلى معايير الجودة' },
            { icon: RefreshCw, title: 'صيانة ودعم', desc: 'صيانة مستمرة ودعم فني لضمان استمرارية أداء موقعك' },
          ]
        : [
            { icon: Sparkles, title: 'Design', desc: 'Award-winning creative design that creates a lasting impression for your brand' },
            { icon: Search, title: 'SEO', desc: 'Search engine optimization to rank your site at the top of search results' },
            { icon: Code, title: 'Development', desc: 'Building with the latest web technologies and highest quality standards' },
            { icon: RefreshCw, title: 'Maintenance & Support', desc: 'Ongoing maintenance and support for continuous performance' },
          ];

    return (
        <>
            <Head title={locale === 'ar' ? 'تطوير المواقع' : 'Web Development'}>
                <meta name="description" content="Professional web design and development services — static and dynamic websites using latest technologies" />
            </Head>

            <Section background="navy" className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
                <div className="absolute inset-0">
                    <img src="/images/services/web-design.jpg" alt="" className="size-full object-cover opacity-20" />
                ...
                <Container className="relative">
                    <div className="mx-auto max-w-3xl text-center">
                        ...
                        <Heading level="h1" className="text-white">
                            {locale === 'ar' ? 'تطوير المواقع' : 'Web Development'}
                        </Heading>
                        ...
                        {phases.map((phase, i) => {
                            ...
                            return (
                                <div key={i} className="rounded-xl border border-neutral-200 bg-white p-6 text-center transition-all hover:border-brand-navy-200 hover:shadow-md">
                                    ...
                                    <Text variant="body-sm" className="mt-2 text-neutral-600">{phase.desc}</Text>
                                </div>
                            );
                        })}
                    </div>
            ...
            <Section>
                <Container className="mx-auto max-w-3xl text-center">
                    <Heading level="h2">
                        {locale === 'ar' ? 'وكالة إبداعية حائزة على جوائز' : 'An Award-Winning Creative Agency'}
                    </Heading>
                    <Text variant="body-lg" className="mt-4 text-neutral-600">
                        {locale === 'ar'
                    ...
                    <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
                        ...
                        <Button asChild size="lg" variant="outline" className="border-brand-navy-200 text-brand-navy-700 hover:bg-brand-navy-50">
