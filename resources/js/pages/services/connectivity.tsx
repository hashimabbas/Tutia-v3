...
import { Wifi, ArrowRight, ArrowLeft, Antenna, Settings, ClipboardCheck } from 'lucide-react';
...
export default function Connectivity() {
    ...
    const solutions = locale === 'ar'
        ? [
            { icon: Antenna, title: 'تغطية الجوال', desc: 'حلول تغطية جوال داخلية/خارجية مع جميع مشغلي الشبكات في السودان' },
            { icon: Wifi, title: 'شبكات Wi-Fi', desc: 'تغطية واي فاي داخلية وخارجية موثوقة للمنازل والشركات' },
            { icon: Settings, title: 'استشارات وصيانة', desc: 'خدمات استشارية وصيانة مستمرة لضمان أفضل أداء' },
          ]
        : [
            { icon: Antenna, title: 'Mobile Coverage', desc: 'Indoor/outdoor mobile coverage solutions with all network operators in Sudan' },
            { icon: Wifi, title: 'Wi-Fi Networks', desc: 'Reliable indoor/outdoor Wi-Fi coverage for homes and businesses' },
            { icon: Settings, title: 'Maintenance & Consulting', desc: 'Ongoing maintenance and consultancy services for optimal performance' },
          ];

    const process = locale === 'ar'
        ? [
            { icon: ClipboardCheck, title: 'تقييم الموقع', desc: 'زيارة ميدانية وتقييم التغطية الحالية لتحديد أفضل الحلول' },
            { icon: Settings, title: 'تصميم النظام', desc: 'تصميم نظام التغطية الأمثل بناءً على نتائج التقييم' },
            { icon: Wifi, title: 'التركيب والتشغيل', desc: 'تركيب أجهزة التغطية وتشغيل النظام بالكامل' },
          ]
        : [
            { icon: ClipboardCheck, title: 'Site Assessment', desc: 'On-site coverage assessment to determine optimal solutions' },
            { icon: Settings, title: 'System Design', desc: 'Design the optimal coverage system based on assessment results' },
            { icon: Wifi, title: 'Installation & Commissioning', desc: 'Install coverage equipment and commission the entire system' },
          ];

    return (
        <>
            <Head title={locale === 'ar' ? 'حلول الاتصال' : 'Connectivity Solutions'}>
                <meta name="description" content="Mobile coverage and Wi-Fi solutions using latest technologies approved by all network operators in Sudan" />
            </Head>

            <Section background="navy" className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
                <div className="absolute inset-0">
                    <img src="/images/services/network-g.jpg" alt="" className="size-full object-cover opacity-20" />
            ...
            <Section>
                <Container>
                    <Heading level="h2" className="text-center">
                        {locale === 'ar' ? 'عملية التنفيذ' : 'Implementation Process'}
                    </Heading>
                    <div className="mt-10 grid gap-6 md:grid-cols-3">
                        {process.map((step, i) => {
                            ...
                            return (
                                <div key={i} className="relative rounded-xl border border-neutral-200 bg-white p-6 transition-all hover:border-brand-navy-200 hover:shadow-md">
                                    ...
                                    <div className="absolute -right-3 -top-3 flex size-8 items-center justify-center rounded-full bg-brand-navy-600 text-sm font-bold text-white">
                                        {i + 1}
                                    </div>
                                    <h3 className="text-lg font-semibold text-neutral-900">{step.title}</h3>
                                    <Text variant="body-sm" className="mt-2 text-neutral-600">{step.desc}</Text>
                                </div>
                            );
                        })}
                    </div>
